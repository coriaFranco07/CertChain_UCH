...
const NftCard = ({ nft }) => {
  ...
  useEffect(() => {
    fetch(imageUrl)
      .then((response) => {
        if (!response.ok) {
          throw new Error('Error en cargar la imagen');
        }
  ...
  return (
    <div style={{ width: '25%', padding: '10px', border: '1px solid #ccc' }}>
      {isVideo ? (
        ...
        <div>El NFT no se puede mostrar</div> // If the image fails to load, display this message.
      ) 
       : (
          <img src={imageUrl} alt={nft.name} style={{ width: '100%' }} onError={(e) => handleImageError(e, setImageError)} />
        )}
      <h3>{nft.name}</h3>
      <p><strong>Simbolo:</strong> {nft.symbol}</p>
